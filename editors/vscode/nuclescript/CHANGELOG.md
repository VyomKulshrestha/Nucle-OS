...
- Added a "Getting started" section to the README with a runnable example
  and a real download link for `nucle-cli` (prebuilt binaries already
  existed on GitHub Releases; nothing previously pointed to them) —
  clarifies that the extension is editor tooling only, and that running a
  program needs `nucle-cli` separately from the auto-downloaded
  `nucle-lsp`. Also documents that the four official `@nuclescript/*`
  packages resolve for hover/diagnostics with no install step, since
  they're compiled directly into `nucle-lsp` and `nucle-cli`.
