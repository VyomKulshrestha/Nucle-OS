...
`confirm-users`/`encrypt-pool`/`decrypt-pool`/`agent` persist to a real pool
directory, so `nucle store` followed by a *separate* `nucle retrieve`
...
| **Total** | **612 (+3 doctests, +1 ignored)** | **End-to-end: binary → DNA → noise → ECC → recover → binary** |
