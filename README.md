...
`nucle store`/`retrieve`/`migrate`/`search`/`pool`/`list`/`agent` persist to
a real pool directory, so `nucle store` followed by a *separate* `nucle
retrieve` invocation genuinely works — not just within one process's own
memory.
...
| `nucle-cli` | 7 | Cross-process pool persistence, the hierarchical namespace, and real concurrent-process safety, spawning the real, compiled `nucle-cli` binary per test (`CARGO_BIN_EXE_nucle-cli`, not an in-memory roundtrip): `store` then `retrieve` in separate processes, a clean not-found error with no prior store, a stray partial `.tmp` file never corrupting a later real invocation, `docs/readme.txt`/`downloads/readme.txt` storing and listing independently by prefix, an absolute source path still storing under its bare leaf name, and two genuinely concurrent `store` processes racing on the same pool never silently losing a write |
...
| **Total** | **561 (+3 doctests, +1 ignored)** | **End-to-end: binary → DNA → noise → ECC → recover → binary** |
