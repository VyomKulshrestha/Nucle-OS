...
`agent` persist to a real pool directory, so `nucle store` followed by a
*separate* `nucle retrieve` invocation genuinely works — not just within
one process's own memory.
...
| **Total** | **578 (+3 doctests, +1 ignored)** | **End-to-end: binary → DNA → noise → ECC → recover → binary** |
