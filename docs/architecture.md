...
**This fixes Illumina, not Nanopore, and that split is a real, distinct limitation, not an oversight.** `build_consensus` aligns reads by raw position (truncating to the shortest/median length and voting base-by-base at each index), which only works when reads are the same length and errors are substitutions. Illumina is exactly that. Nanopore is indel-dominant: an insertion or deletion shifts every base after it out of position, so positional majority voting across Nanopore reads compares unrelated bases and produces garbage rather than a correction — verified directly (`nucle_vfs::tests::test_nanopore_indels_still_break_positional_consensus`), the roundtrip still fails under Nanopore noise at 10x coverage regardless of redundancy. Fixing this for real needs sequence alignment before voting (e.g. Needleman-Wunsch pairwise alignment or partial-order alignment across all copies in a group), not more coverage or more parity — those only help once the reads are actually comparable position-by-position.
