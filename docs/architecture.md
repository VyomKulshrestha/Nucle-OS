...
**Capacity limits.** `PersistedState`/`NucleOS` carry an optional `max_nucleotides` (`None` by default — unbounded, matching earlier behavior). `nucle capacity [max] [--unlimited]` reports current usage/limit when called with neither argument, or sets/clears the limit (persisting it so later invocations against the same `pool_dir` enforce it too — the limit is pool-level configuration, not a per-invocation flag). `dna_write` checks the incoming file's real, final strand-and-parity nucleotide count — computed *after* codec/ECC encoding, not estimated from raw input size — against `used + incoming` before touching the pool at all, so a refused write leaves no partial strands and no catalog entry. The error message ("pool capacity exceeded: ... needs N more nucleotides, but only M of MAX remain") is deliberately distinct from primer-library exhaustion's older, unrelated "no primer pairs available" message, since the two failure causes were previously indistinguishable from the CLI's own output.

**Audit log.** `NucleOS` carries an optional `pool_dir` (set by `open()`, left `None` for an ephemeral `NucleOS::new()` instance) so `dna_write`/`dna_read`/`dna_delete` can append one JSON line to `pool_dir/audit.log` per call, success or failure — an append-only file (`nucle_vfs::audit`), never rewritten, so a crash immediately after an operation can at worst leave one truncated trailing line, never corrupt an earlier entry (a line that fails to parse on read is skipped, not fatal). Each event carries a timestamp, the operation (`write`/`read`/`delete`), the filename, the archive ID if one was resolved (`None` on a failure before an ID existed, e.g. "file not found"), whether it succeeded, and a short human-readable detail. A migration doesn't get its own event kind: `migrate_object` is implemented as a `dna_read` + `dna_delete` + `dna_write` against the same filename, and each of those three already appends its own event, so a migration shows up as that same three-event trail with no extra bookkeeping. An audit-log write failure (e.g. disk full) doesn't fail the underlying operation — it's swallowed, deliberately: this is an observability trail, not a data-durability guarantee like `persist()`, so losing one audit line shouldn't lose a file that was actually stored (or refuse a read/delete that otherwise succeeded). `nucle audit [--tail N]` reads the log back, oldest first (or only the most recent N).
...
**Rewrite/versioning.** `dna_write` on a filename that already exists used to be rejected outright ("file '<name>' already exists") — genuinely write-once at the file level, independent of whether the underlying medium (in `nucle_blockdev`'s terms) is write-once or rewritable. That's gone: a repeat write now stores the new content as the *next version* rather than erroring. `Catalog` gained a `version_history: HashMap<filename, Vec<DnaFile>>` alongside its existing current-only `files`/`name_index` maps — `register()` archives whatever was current into that filename's history instead of deleting it, so `list()`/`dna_stat()`/`dna_list()` still only ever see one entry per filename (the current version), not one per historical write, with zero change to their own callers. Nothing physically disappears on a rewrite: the new version's strands land under a fresh, content-addressed `file_id` and a fresh primer (both already naturally unique per write, before this feature existed), so the old version's strands simply keep occupying their own, still-addressable slot in the pool — a rewrite is additive to the pool's real footprint, not a strand-for-strand swap, matching a real "write another copy, don't erase the old one" medium far more honestly than an in-place mutation would. `dna_history(filename)` lists every version oldest-first, ending with the current one; `dna_read_version(filename, version)` runs the exact same CRISPR/consensus/RS/codec decode pipeline `dna_read` does, just against a specific version's own `DnaFile` (both share a new private `decode_dna_file` rather than duplicating that pipeline). `dna_delete` was changed to remove the *entire* version chain (`Catalog::take_all_versions`, `DeleteResult::versions_removed`) — a history entry with no current version to point back to would be an orphan nothing else in this crate expects, so there's no "delete just the current version, keep history around headless" mode. A superseded version's search index entry is dropped at write time (search should only ever surface a filename's current version, the same "one visible entry per name" principle `Catalog::register` already applies), while its catalog/pool data stays fully intact.
...
Every adapter's HTTP calls go through a shared `ureq::Agent` (`http_client::new_agent`) with a bounded 5-second global timeout, since `Provider`'s synchronous interface has no outer async timeout layer to fall back on — an unreachable vendor endpoint fails a `submit`/`status` call cleanly instead of blocking the caller indefinitely. Each adapter is tested against a local `tiny_http` server standing in for the real vendor (a genuine HTTP round-trip through the real request-building/response-parsing code, just not the real vendor's servers) rather than mocking `ureq` itself.
...
**Verified:** 22 new `nucle_blockdev` tests — the `BlockDevice` contract (out-of-range/wrong-size/never-written rejection) against both backends; `SynthesisArrayBlockDevice`'s write-once enforcement and a real corruption-then-recovery round trip; `RewritableNanoporeBlockDevice`'s overwrite-in-place, per-operation latency on both reads and writes, and its own corruption-then-recovery round trip; and `blob`'s round trip across both backends plus an empty-blob and a too-large-blob rejection. Two new `[profile.dev.package.*]` opt-level overrides (`nucle_synth`, `nucle_blockdev`) were needed for the same reason the pre-existing `nucle_ecc`/`nucle_codec` ones were: coverage-depth noise injection and consensus/RS decoding are CPU-heavy enough that debug-mode tests would otherwise be needlessly slow. Full `cargo test --workspace` re-verified green (655 tests total, up from 633), confirmed stable across two separate full-workspace runs (the batched-write timing test was rewritten to assert relative latency *savings* against a same-config sequential baseline rather than a fixed wall-clock threshold specifically because the first version was flaky under full-workspace-parallel CPU contention).
