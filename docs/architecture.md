...
**Illumina works. Nanopore still doesn't, and the reason changed once we dug in — worth documenting precisely rather than leaving the first explanation stale.** The original diagnosis was that `build_consensus` voted by raw position, which breaks under indels. That's fixed: it now globally aligns (Needleman-Wunsch) any read whose length differs from the group's reference before voting, so a frame-shifted read still lands its votes on the right column (`nucle_ecc::consensus::tests::test_consensus_corrects_frame_shifting_indels`). A second, separate bug turned out to be the *dominant* blocker: `nucle_index::primer::PrimerPair::{matches_forward, untag_strand}` matched primers by exact position, so a single indel landing inside a primer — routine at Nanopore's ~4%/base indel rate — made CRISPR retrieval treat the whole strand as non-target and drop it before it ever reached consensus. Both primer functions now locate boundaries by bounded edit-distance search instead (`nucle_index::primer::tests::test_untag_tolerates_*`).

With both fixed, Nanopore recovery gets further than before but still fails end-to-end at realistic settings (`nucle_vfs::tests::test_nanopore_still_fails_at_realistic_indel_density_despite_alignment_fixes`, verified to still fail even at 50x coverage and 12 parity strands — ruling out "just needs more redundancy"). The remaining cause: a ~150nt strand at Nanopore's error rate accumulates *many* simultaneous indels per read (6-10, empirically), and each read is realigned pairwise against a single arbitrarily-picked noisy read rather than against the true sequence or a proper multi-read consensus. At that indel density, pairwise realignment against one noisy anchor accumulates drift and produces wrong votes scattered across the strand, not just at the indel site. Fixing this for real needs multi-read consensus built from all copies at once — partial-order alignment, the approach real long-read polishers like Racon/Medaka use — rather than each read independently against one reference. That's a substantially larger piece of work than adding alignment-awareness to pairwise voting, and remains open.
