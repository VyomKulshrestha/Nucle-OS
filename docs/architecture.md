...
With all of that fixed, Nanopore recovery still fails end-to-end at realistic settings (`nucle_vfs::tests::test_nanopore_still_fails_at_realistic_indel_density_despite_alignment_fixes`, verified at 50x coverage and 12 parity strands — ruling out "just needs more redundancy"), though it no longer *crashes* there either, which it did briefly during development. A synthetic stress test (30 reads, 3-6 edits each over a 43nt sequence — a higher combined edit rate than Nanopore's own ~7%) gets to 42/43 bases correct in a single POA pass; a naive attempt at iterative refinement (reseed from the first pass's own result, like Racon/Medaka's polishing rounds) reduced but didn't eliminate that residual, and doing it caused a real regression elsewhere (a double-counted vote weight briefly broke the working Illumina case) that had to be caught and reverted. Multi-round polishing done correctly, or a more sophisticated graph-alignment banding scheme, remains open work — a single POA pass is a large, verified improvement over pairwise realignment, but not yet sufficient to close out Nanopore fully.
